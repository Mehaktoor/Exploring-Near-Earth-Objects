...
"""
Now we will create filters for all the filters - Distance, Date,
Velocity, Diameter and Hazardous.
"""
...
class filterDiameter(AttributeFilter):
    ...
    def get(cls, approach):
        # this will override its superclass's get function.
        # This is NearEarthObject's instance - hence neo
...
def create_filters(date=None, start_date=None, end_date=None,
    ...
    occured on exactly that given date. Similarly, the `--min-distance`
    option corresponds to the `distance_min` argument, and represents a
    filter that selects close approaches whose nominal approach distance
    is at least that far away from Earth. Each option is `None` if not
    specified at the command line (in particular, this means that the
    `--not-hazardous` flag results in `hazardous=False`, not to be
